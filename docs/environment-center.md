...
Smart configuration persists its mode with the existing v1 workspace state; older state without that field defaults to manual. A reload restores the mode but requires fresh consent and scanning. Target changes invalidate the scan, selected-software changes recalculate the budget, and both copy and download requery capacity before exporting. The export is canceled if the plan changes while verification is running.
