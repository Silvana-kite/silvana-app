...
export class SourceHttp {
  ...
  async read(url: string): Promise<CachedPage> {
    ...
    if (!sourceHosts.has(host) || new URL(url).protocol !== 'https:') throw new Error('Unregistered source URL');
  ...
  private async fetchPage(url: string): Promise<CachedPage> {
    ...
    for (let redirects = 0; redirects <= 5; redirects++) {
      this.requestCount++;
      response = await fetch(target, { headers, signal, redirect: 'manual' });
      if (![301, 302, 303, 307, 308].includes(response.status)) break;
      const next = new URL(response.headers.get('location') ?? '', target);
      if (next.protocol !== 'https:' || !sourceHosts.has(next.hostname)) throw new Error('Source redirected outside registered hosts');
