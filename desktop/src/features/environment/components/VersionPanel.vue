...
const localVersions = computed(() => installable.value.filter(v => `${v.version} ${v.label ?? ''}`.toLowerCase().includes(search.value.toLowerCase())));
const installId = (version: string) => installable.value.find(v => !v.label && v.version === version)?.id;
  ...
  <Teleport to="body">
    <div class="release-overlay" @click.self="emit('close')">
      <section ref="dialog" class="release-panel" role="dialog" aria-modal="true" :aria-label="`${tool.name} 版本`" @keydown="keydown">
        ...
        <div v-if="localVersions.length" class="release-local"><h3>可加入安装方案</h3><div v-for="version in localVersions" :key="version.id" class="release-row"><span>{{ version.label ?? version.version }} <small v-if="version.recommended">推荐</small><small v-if="version.managedByToolId === 'volta'">通过 Volta 管理，选择后会加入 Volta</small></span><button type="button" @click="choose(version.id)">{{ selectedVersion === version.id ? '已选择' : '选择此版本' }}</button></div></div>
        ...
        <div class="release-list" :aria-busy="loading">
          <article v-for="version in data?.items ?? []" :key="`${version.originalVersion}:${version.build}:${version.releaseTrack}`" class="release-row">
            ...
            <div class="release-actions"><button v-if="!version.withdrawn && !version.isPrerelease && installId(version.version)" type="button" @click="choose(installId(version.version)!)">{{ installable.find(v => v.id === installId(version.version))?.managedByToolId === 'volta' ? '通过 Volta 安装此版本' : '选择此版本' }}</button><span v-else class="release-manual">仅官方下载</span><a :href="version.pageUrl" @click.prevent="openOfficialUrl(version.pageUrl)">{{ version.assets.length ? '官方发布 / 下载页' : '官方发布记录' }} ↗</a>
