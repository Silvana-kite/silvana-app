...
const statusText = computed(() => offline.value ? '离线缓存，仅包含此前加载过的查询页' : data.value?.status === 'stale' ? '当前显示最近成功采集的数据' : data.value?.status === 'pending' ? '历史版本尚在采集中' : data.value?.status === 'unavailable' ? '历史版本服务暂不可用' : data.value?.status === 'unsupported' ? '此工具暂未接入历史采集' : '官方历史版本');
async function fetchPage(refresh = false) {
  const request = ++sequence; loading.value = true; error.value = '';
  try {
    const result = await loadReleases(props.tool.id, { q: search.value.trim(), page: page.value }, refresh);
...
watch(search, () => { clearTimeout(timer); page.value = 1; data.value = undefined; sequence++; timer = setTimeout(() => void fetchPage(), 250); });
    ...
    <div class="release-overlay" @click.self="emit('close')">
      <section ref="dialog" class="release-panel" role="dialog" aria-modal="true" :aria-label="`${tool.name} 版本`" @keydown="keydown">
        ...
        <div class="release-list" :aria-busy="loading">
          <article v-for="version in data?.items ?? []" :key="version.version" class="release-row">
            <div><a :href="version.pageUrl" @click.prevent="openOfficialUrl(version.pageUrl)">{{ version.version }} ↗</a><small>{{ version.channel === 'eol' ? '已结束支持 · ' : version.channel === 'lts' ? 'LTS · ' : '' }}{{ version.releaseDate?.slice(0, 10) }}</small><small v-if="version.bundledNpm">附带 npm {{ version.bundledNpm }}</small></div>
            <div class="release-actions"><button v-if="installId(version.version)" type="button" @click="choose(installId(version.version)!)">选择此版本</button><span v-else class="release-manual">需手动下载</span><a :href="version.pageUrl" @click.prevent="openOfficialUrl(version.pageUrl)">{{ version.assets.length ? '官方发布 / 下载页' : '官方发布记录' }} ↗</a>
              <details v-if="version.assets.length"><summary>下载资源（{{ version.assets.length }}）</summary><a v-for="file in version.assets" :key="file.url" :href="file.url" @click.prevent="openOfficialUrl(file.url)">{{ file.kind === 'source' ? '源码 / 包归档' : '安装资源' }} · {{ file.name }}{{ file.platform && platform && file.platform !== platform ? ' · 其他系统' : '' }} ↗</a></details>
            </div>
          </article>
          <p v-if="!loading && data && !data.items.length">{{ search ? '没有找到匹配版本' : '暂时没有可用的历史数据' }}</p>
        </div>
        <footer><span>共 {{ data?.total ?? 0 }} 个版本 · 第 {{ page }} 页</span><div><button type="button" :disabled="loading || page === 1" @click="changePage(page - 1)">上一页</button><button type="button" :disabled="loading || !data || page * data.pageSize >= data.total" @click="changePage(page + 1)">下一页</button></div></footer>
      </section>
    </div>
  </Teleport>
</template>
